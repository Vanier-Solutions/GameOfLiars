...
router.post('/kick', (req, res) => {
    try {
        const { code, hostPlayerId, playerToKick } = req.body;
        
        console.log('Kick request:', { code, hostPlayerId, playerToKick });
        
        const lobby = activeLobbies.get(code.toUpperCase());
        if (!lobby) {
            console.log('Lobby not found:', code.toUpperCase());
            return res.status(404).json({ error: 'Lobby not found' });
        }
        
        // Verify the player making the request is the host by checking their ID
        const hostPlayer = lobby.getPlayerById(hostPlayerId);
        console.log('Host player found:', hostPlayer ? hostPlayer.getName() : 'not found');
        console.log('Lobby host:', lobby.getHost().getName());
        
        if (!hostPlayer || hostPlayer.getName() !== lobby.getHost().getName()) {
        ...
        console.log('Player to kick found:', playerToKickObj ? playerToKickObj.getName() : 'not found');
        
        if (!playerToKickObj) {
            console.log('Player to kick not found:', playerToKick);
            return res.status(404).json({ error: 'Player to kick not found' });
        }
        
        // Cannot kick the host
        if (playerToKickObj.getName() === lobby.getHost().getName()) {
            console.log('Cannot kick host');
        ...
        console.log('Removing player:', kickedPlayerName, 'with ID:', kickedPlayerId);
        ...
        const gameEvents = req.app.locals.gameEvents;
        if (gameEvents) {
            console.log('Broadcasting player kicked event');
            ...
            console.log('Game events not available');
        }
