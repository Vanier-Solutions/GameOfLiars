...
function TeamCard({
  ...
  players,
  currentPlayerId,
}: { side: "Blue" | "Red"; color: "blue" | "red"; score: number; players: Player[]; currentPlayerId?: string }) {
  ...
  return (
    <div className={`rounded-2xl shadow-xl border border-white/10 overflow-hidden`}>
      <div
        className={`px-4 py-3 text-white font-semibold bg-gradient-to-r ${colorClasses} flex items-center justify-between`}
      >
        <span>{side} Team</span>
  ...
  return (
    <div className={`${messageBg} rounded-lg p-1.5 mb-1.5`}>
      <div className="flex items-center gap-2 text-sm">
        <span
          className={`${isTeamChat ? teamBgColors[message.team] : "bg-white"} ${isTeamChat ? "text-white" : teamColors[message.team]} px-2 py-0.5 rounded font-medium text-xs`}
        >
...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    const handleTeamAnswerSubmitted = (data: {
      team: string
      isSteal: boolean
      ...
      <header className="relative z-10 border-b border-white/10 bg-slate-950/40 backdrop-blur-md">
        <div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between">
          ...
          <div className="text-xs sm:text-sm text-white/70">
            Room: {code} • Round {round}/{maxScore}
          </div>
        </div>
      </header>
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-900/60 backdrop-blur-xl shadow-2xl p-8 text-center min-h-[300px] flex flex-col justify-center">
                ...
                <div className="space-y-4">
                  ...
                  {currentPlayer?.isHost ? (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={startRound}
                      className="mx-auto inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 px-6 py-3 font-semibold shadow-lg"
                    >
              ...
              {phase === "question" && (
                <div className="space-y-5 text-left">
                  ...
                  <div className="text-xl sm:text-2xl font-bold text-center leading-snug max-w-3xl mx-auto">
                    {question}
                  </div>
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      placeholder="Type your team answer"
                      className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500/60"
                    />
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={() => submitAnswer(false)}
                      className="rounded-xl bg-emerald-600 hover:bg-emerald-500 px-5 py-3 font-semibold shadow-lg inline-flex items-center gap-2"
                    >
                    ...
                    <motion.button
                      ...
                      className="rounded-xl bg-rose-600 hover:bg-rose-500 px-5 py-3 font-semibold shadow-lg"
                    >
                      Steal
                    </motion.button>
                  </div>
              ...
              {phase === "answered" && (
                <div className="space-y-3">
                  ...
                  <div className="text-md text-white/60">Time left: {secondsLeft}s</div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-indigo-600/20 rounded-xl p-4 border border-indigo-500/30">
                      ...
                      <div className="text-white text-lg">
                        {roundResults.blueSteal ? "🎯 STEAL" : roundResults.blueAnswer}
                      </div>
                    ...
                    <div className="bg-rose-600/20 rounded-xl p-4 border border-rose-500/30">
                      ...
                      <div className="text-white text-lg">
                        {roundResults.redSteal ? "🎯 STEAL" : roundResults.redAnswer}
                      </div>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="inline-flex rounded-lg bg-white/5 p-1 border border-white/10">
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-md transition-colors ${chatMode === "game" ? "bg-indigo-600 text-white" : "text-white/70 hover:text-white"}`}
                  >
                    All
                  </button>
                  <button
                    onClick={() => setChatMode("team")}
                    className={`px-3 py-1 text-xs rounded-md transition-colors ${chatMode === "team" ? "bg-emerald-600 text-white" : "text-white/70 hover:text-white"}`}
                  >
                    Team
                  </button>
                </div>
